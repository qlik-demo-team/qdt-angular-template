...
//   kpi1props = {qMeasure: `Count( {$<Priority={'High'}, Status -={'Closed'} >} Distinct %CaseId )`, listenToSelections: false};
  ...
  jsContent=`
    ...
    viz2props = {id: '298bbd6d-f23d-4469-94a2-df243d680e0c', height:'300px'};
    viz3props = {id: 'ycppXj', height:'200px'};
    kpi1props = {qMeasure: "Count( {$<Priority={'High'}, Status -={'Closed'} >} Distinct %CaseId )", listenToSelections: true};
    kpi2props = {qMeasure: "Count( {$<Priority={'Medium'}, Status -={'Closed'} >} Distinct %CaseId )", listenToSelections: true};
    kpi3props = {qMeasure: "Count( {$<Priority={'Low'}, Status -={'Closed'} >} Distinct %CaseId )", listenToSelections: true};
